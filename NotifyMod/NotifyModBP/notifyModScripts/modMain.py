...
class NotifyTestMod(object):

    def __init__(self):
        logger.info("===== init %s mod =====", modConfig.MOD_NAMESPACE)

    @Mod.InitServer()
    def NotifyTestModServerInit(self):
    ...
    @Mod.DestroyServer()
    def NotifyTestModServerDestroy(self):
        logger.info("===== destroy %s server =====", modConfig.SERVER_SYSTEM_NAME)
    
    @Mod.InitClient()
    def NotifyTestModClientInit(self):
    ...
    @Mod.DestroyClient()
    def NotifyTestModClientDestroy(self):
